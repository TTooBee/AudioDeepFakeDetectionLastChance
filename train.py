...
def load_audios(wav_files, frame_size, sample_rate):
    ...
    mfcc_many_frames_all_tensor = torch.cat(mfcc_frames_all, dim=2)  
    print(f'shape of mfcc_many_frames_all_tensor : {mfcc_many_frames_all_tensor.shape}')  
    
    for i in range(mfcc_many_frames_all_tensor.shape[2]):
        if (i+100 > mfcc_many_frames_all_tensor.shape[2]):
            break
        if (i%100 == 0):
            mfcc_many_frames_all.append(mfcc_many_frames_all_tensor[:, :, i:i+100])
                
    print(f'shape of mfcc_many_frames_all_tensor : {mfcc_many_frames_all_tensor.shape[2]}')       
    print(f'length of list mfcc_frames_all : {len(mfcc_frames_all)}')
    
    mfcc_many_frames_all_tensor = torch.cat(mfcc_many_frames_all, dim=0)
    mfcc_frames_all_tensor = torch.cat(mfcc_frames_all, dim=0)
    
    return mfcc_many_frames_all_tensor
...
# 여기서부터 학습    
real_audio_path = "../AudioDeepFakeDetection/real_low/wav"
fake_audio_path = "../AudioDeepFakeDetection/fake_low/wav"
...
for epoch in range(epochs):
    ...
    writer.add_scalar('Acc/train', avg_train_accuracy, epoch)
    writer.add_scalar('Acc/val', avg_val_accuracy, epoch)
    
    print(f"Epoch {epoch+1}/{epochs}, Training Loss: {avg_train_loss:.4f}, Validation Loss: {avg_val_loss:.4f}, Training Acc: {avg_train_accuracy:.4f}, Validation Acc: {avg_val_accuracy:.4f}")
